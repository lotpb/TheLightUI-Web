...
// Draft/Sent/Accepted/Declined are real, draggable statuses. Expired is a
// computed overlay (a 'sent' proposal past its expiresDate) — shown for
// visibility but not a drop target, same reasoning as Invoices' Overdue.
const COLUMNS: { id: ProposalStatus; label: string; barClass: string; textClass: string; badgeClass: string; droppable: boolean }[] = [
  ...
  { id: 'accepted', label: 'Accepted', barClass: 'bg-green-500',  textClass: 'text-green-400',  badgeClass: 'bg-green-600',  droppable: true },
  { id: 'declined', label: 'Declined', barClass: 'bg-red-500',    textClass: 'text-red-400',    badgeClass: 'bg-red-600',    droppable: true },
  { id: 'expired',  label: 'Expired',  barClass: 'bg-amber-500',  textClass: 'text-amber-400',  badgeClass: 'bg-amber-600',  droppable: false },
]
...
export default function ProposalPipelinePage() {
  ...
  const filtered = useMemo(() => {
    ...
    return proposals.filter(p =>
      p.customerName.toLowerCase().includes(q) || p.proposalNumber.toLowerCase().includes(q),
    )
  }, [proposals, search])

  const columns = useMemo(() => {
    const buckets: Record<ProposalStatus, Proposal[]> = { draft: [], sent: [], accepted: [], declined: [], expired: [] }
    for (const p of filtered) buckets[effectiveStatus(p)].push(p)
    for (const k of Object.keys(buckets) as ProposalStatus[]) {
      buckets[k].sort((a, b) => b.updatedAt.getTime() - a.updatedAt.getTime())
    }
    return buckets
  }, [filtered])

  const acceptedValue = useMemo(() => columns.accepted.reduce((s, p) => s + proposalTotal(p), 0), [columns])
  ...
  function onDragOverCol(e: React.DragEvent, col: ProposalStatus, droppable: boolean) {
    if (!droppable) return
    e.preventDefault()
    setDragOverCol(col)
  }
  ...
  async function onDropCol(e: React.DragEvent, target: ProposalStatus, droppable: boolean) {
    e.preventDefault()
    setDragOverCol(null)
    if (!droppable) return
    const id = draggingIdRef.current
    if (!id) return
    await moveProposal(id, target)
  }
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {acceptedValue > 0 && (
            <span className="text-xs text-green-400 font-medium bg-green-900/30 border border-green-700/40 rounded-full px-3 py-1.5">
              {fmtCurrency(acceptedValue)} accepted
            </span>
          )}
              ...
              >
                ...
                <div className="flex flex-col gap-2 px-2 pb-3 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 260px)' }}>
                  {shown.length === 0 && !isOver ? (
                    <p className="text-xs text-gray-500 text-center py-8">No proposals</p>
                  ) : (
                    shown.map(p => (
                      <ProposalCard
                        key={p.id}
                  ...
                  {overflow > 0 && (
                    <Link to="/proposals" className="text-xs text-center text-indigo-400 hover:text-indigo-300 py-2 transition-colors">
                      +{overflow} more →
                    </Link>
                  )}
                </div>
...
function ProposalCard({
  proposal: p, col, isDragging, onDragStart, onDragEnd, onMove, onConvert, converting,
}: {
  proposal: Proposal
  col: ColumnConfig
  isDragging: boolean
  ...
  return (
    ...
    >
      <div className="flex items-start">
        ...
        >
          <div className="flex items-baseline justify-between gap-2">
            ...
            <p className="text-sm font-semibold text-white shrink-0">{fmtCurrency(proposalTotal(p))}</p>
          </div>
          <div className="flex items-center justify-between gap-2">
            <p className="text-xs text-gray-500 truncate">{p.proposalNumber}</p>
            <p className="text-xs text-gray-500 shrink-0">Exp {fmtDate(p.expiresDate)}</p>
          </div>
        </Link>
      ...
      {col.id === 'accepted' && !p.convertedInvoiceId && (
        <button
          onClick={onConvert}
          disabled={converting}
          className="w-full text-xs px-3 py-1.5 bg-indigo-600/20 text-indigo-300 border-t border-indigo-700/30 hover:bg-indigo-600/30 transition-colors disabled:opacity-40"
        >
          {converting ? 'Converting…' : '🧾 Convert to Invoice'}
        </button>
      )}
      {p.convertedInvoiceId && col.id === 'accepted' && (
        <Link
          to={`/invoices/${p.convertedInvoiceId}`}
          className="block w-full text-center text-xs px-3 py-1.5 bg-emerald-600/20 text-emerald-300 border-t border-emerald-700/30 hover:bg-emerald-600/30 transition-colors"
        >
          View Invoice →
        </Link>
      )}
