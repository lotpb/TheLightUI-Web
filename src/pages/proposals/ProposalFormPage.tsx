...
import { validateProposalForm } from '../../validation/proposalFormSchema'
...
function expiresDefault(): Date {
  const d = new Date()
  d.setDate(d.getDate() + 30)
  return d
}

const emptyLine = (): ProposalLineItem => ({ description: '', qty: 1, rate: 0 })
...
export default function ProposalFormPage() {
  ...
  useEffect(() => {
    ...
    getProposal(id).then(p => {
      ...
      setIssueDate(dateToInput(p.issueDate))
  ...
  // Pre-fill from customer (new proposal from customer detail)
  useEffect(() => {
  ...
  function updateLine(idx: number, field: keyof ProposalLineItem, value: string) {
    setLineItems(prev => {
      const next = [...prev]
  ...
  async function handleSave(status: Proposal['status']) {
    const finalCustomerName = customerName.trim() || customerQuery.trim()
    const validationError = validateProposalForm({ customerId, customerName: finalCustomerName, lineItems })
    if (validationError) {
      toast(validationError, 'error')
      return
    }
    setSaving(true)
    const data: Omit<Proposal, 'id' | 'createdAt' | 'updatedAt'> = {
      companyId: companyId ?? '',
      customerId,
      ...
      expiresDate: inputToDate(expiresDate),
      status,
      lineItems,
      notes,
      taxRate,
    ...
    try {
      if (isEdit && id) {
        ...
        const newId = await createProposal(data)
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-5">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white">{isEdit ? 'Edit Proposal' : 'New Proposal'}</h1>
          <p className="text-sm text-gray-400 mt-0.5">{proposalNumber}</p>
        </div>
      ...
      <div className="card overflow-hidden">
        ...
        <div className="p-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div>
            <label className="text-xs text-gray-500 mb-1 block">Proposal #</label>
            <input type="text" value={proposalNumber} onChange={e => setProposalNumber(e.target.value)} className="input-field w-full text-sm py-1.5 font-mono" />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="date" value={expiresDate} onChange={e => setExpiresDate(e.target.value)} className="input-field w-full text-sm py-1.5" />
          </div>
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
        ...
        <div className="p-4">
          <textarea
            ...
            rows={3}
            placeholder="Scope, terms, special instructions…"
